...
function RecapVersions() {
  ...
  const fetchRecapVersions = async () => {
    try {
      const response = await axiosInstance.get('/api/recap/get-all-versionnotdraft');
  ...
  const createReview = async (recapVersionId) => {
    try {
      ...
      const response = await axiosInstance.post("/api/review/createreview", newReview);
  ...
  return (
    <Box sx={{ width: "80vw" }}>
      <Typography variant='h5' margin={1}>Danh sách bài viết</Typography>

      {/* Search and Filter */}
      <Box sx={{ display: "flex", justifyContent: "space-between", marginBottom: 2 }}>
        <TextField
          label="Tìm kiếm theo tên bài viết hoặc tên sách"
          variant="outlined"
          size="small"
          value={searchTerm}
